...
function typeDataBill(){
  $data=array();
  require "assets/config/php/config.php";
  $requestSQL="SELECT id, name FROM company";
  $requete = $pdo->prepare($requestSQL);
  $requete->execute();
  $company = $requete->fetchAll();
  $requete->closeCursor();
  $requestSQL="SELECT id, CONCAT(lastname,' ',firstname) AS nom FROM person";
  $requete = $pdo->prepare($requestSQL);
  $requete->execute();
...
function billUpdate(){
        ...
        {
            $number=$_GET['number'];
            $requestSQL="UPDATE bill SET date=:date, object=:object, company=:company, person=:person where number = $number";
            $requete = $pdo->prepare($requestSQL);
            $requete->bindParam(":date", $_POST['date']);
            $requete->bindParam(":object", $_POST['object']);
            $requete->bindParam(":company", $_POST['company']);
        ...
        $selectCompany=array();
        $selectPerson=array();
            $requestSQL="SELECT bill.* from bill where number = $number";
            $requete = $pdo->prepare($requestSQL);
            $requete->execute();
            $bill = $requete->fetch();
            $requete->closeCursor();
